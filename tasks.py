...
def extractPublicConstStringPairs(text):
    ...
    for match in re.findall(
        'public\\s+const\\s+string\\s+(\\w+)\\s*=\\s*"([^"]+)";', text
    ):
...
def getPdfSharpWpfVersion():  # kenjiuno.PdfSharp-WPF
    text = getPackageVersionFrom(
        os.path.join(dirname, "PDFsharp/code/PdfSharp/PdfSharp-WPF.csproj")
    )
    return text
...
def getPdfSharpGdiVersion():  # kenjiuno.PdfSharp-GDI
    text = getPackageVersionFrom(
        os.path.join(dirname, "PDFsharp/code/PdfSharp/PdfSharp.csproj")
    )
    return text
...
def setPdfSharpWpfVersion(version):  # kenjiuno.PdfSharp-WPF
    if True:
        ...
        text = updatePublicConstStringPairs(
            text,
            {
    ...
    if True:
        filePath = os.path.join(dirname, "PDFsharp/code/PdfSharp/Doxyfile-wpf")
        text = readAllText(filePath)
...
def setPdfSharpGdiVersion(version):  # kenjiuno.PdfSharp-GDI
    ...
    if True:
        filePath = os.path.join(dirname, "PDFsharp/code/PdfSharp/Doxyfile-gdi")
        text = readAllText(filePath)
        text = updateDoxyfileVar(text, "PROJECT_NUMBER", version)
        writeAllText(filePath, text)
...
def getPdfSharpXpsVersion():  # kenjiuno.PdfSharp.Xps
    text = getPackageVersionFrom(
        os.path.join(dirname, "PDFsharp/code/PdfSharp.Xps/PdfSharp.Xps.csproj")
    )
    return text
...
def updatePackageVersion(text, elementName, value):
    text = re.sub(
        "<" + re.escape(elementName) + ">[^<]+</" + re.escape(elementName) + ">",
...
def setPdfSharpXpsVersion(version):  # kenjiuno.PdfSharp.Xps
    ...
    if True:
        filePath = os.path.join(dirname, "PDFsharp/code/PdfSharp.Xps/Doxyfile")
        text = readAllText(filePath)
        text = updateDoxyfileVar(text, "PROJECT_NUMBER", version)
        writeAllText(filePath, text)
...
def pack(c):
    ...
    def dotnetPack(csproj):
        ...
        c.run(
            "dotnet pack %s.csproj -p:SymbolPackageFormat=snupkg -c Release --include-symbols --include-source"
            % (csproj,),
            env=env,
            echo=True,
        )
